...
const FOOTER_LOGO_SRC =
  "/marketing/characters/BRILLIANCE%20AI%20(1920%20x%20768%20px)%20(3).png";
...
const FOOTER_SOCIALS = [
  ...
  { label: "TikTok", href: "#", icon: "tiktok", color: "#FFFFFF" },
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-10">
        ...
        <div className="py-16 grid grid-cols-1 md:grid-cols-12 gap-12">
          ...
          <div className="md:col-span-4">
            <div className="mb-5">
              <div className="relative h-36 w-[360px] shrink-0 md:h-24 md:w-[240px]">
                <Image
                  src={FOOTER_LOGO_SRC}
                  alt="BrillianceAI"
                  fill
          ...
          <div className="md:col-span-8 flex md:justify-end">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-12 md:gap-16">
              {Object.entries(FOOTER_LINKS).map(([category, links]) => (
                <div key={category}>
                  <h4 className="text-[11px] font-medium uppercase tracking-[0.12em] text-white/20 mb-5">
                    {category}
                  </h4>
                  <ul className="flex flex-col gap-2.5">
                    {links.map((link) => (
                      <li key={link.label}>
                        <a
                          href={link.href}
                          className="text-[13px] text-white/20 hover:text-white/50 transition-colors duration-200 font-normal tracking-[-0.01em]"
                        >
                          {link.label}
                        </a>
                      </li>
        ...
        <div className="py-6 flex flex-col md:flex-row justify-between items-center gap-3">
          <span className="text-[11px] text-white/12 font-normal">
            &copy; {new Date().getFullYear()} BrillianceAI Tech. All rights reserved.
          </span>
          <div className="flex gap-5">
            <a
              href="/privacy"
              className="text-[11px] text-white/12 hover:text-white/25 transition-colors font-normal"
            >
              Privacy Policy
            </a>
            <a
